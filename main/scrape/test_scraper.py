from basketball_reference_links import BasketBallReferenceLinks
...
#     @staticmethod
#     def save_to_csv(dataframe, year, filename):
...
#         # Save the DataFrame to a CSV file in the year's subfolder
#         filepath = os.path.join(year_folder, filename)
#         dataframe.to_csv(filepath, index=False)

#     def scrape_all_years(self, year):
#         team_abv = "PHI"
#         player_name = "James Harden"

#         team_roster = self.scraper.get_team_roster(year, team_abv)
#         self.save_to_csv(team_roster, f"team_roster_{year}_{team_abv}.csv")

#         suffix_ = self.scraper.get_player_suffix(player_name).split(".")[0]
#         player_game_log = self.scraper.get_player_game_log(suffix_, year)
#         self.save_to_csv(player_game_log, f"player_game_log_{year}_{player_name.replace(' ', '_')}.csv")

#         injuries = self.scraper.get_injury_report()
#         self.save_to_csv(injuries, f"injury_report_{year}.csv")

#         player_per_game_stats = self.scraper.get_player_per_game_stats(year)
#         self.save_to_csv(player_per_game_stats, f"player_per_game_stats_{year}.csv")

#         player_per_36_stats = self.scraper.get_player_36_stats(year)
#         self.save_to_csv(player_per_36_stats, f"player_per_36_stats_{year}.csv")

#         player_per_100_stats = self.scraper.get_player_100_stats(year)
#         self.save_to_csv(player_per_100_stats, f"player_per_100_stats_{year}.csv")

#         player_play_by_play_stats = self.scraper.get_player_play_by_play_stats(year)
#         self.save_to_csv(player_play_by_play_stats, f"player_play_by_play_stats_{year}.csv")

#         player_advanced_stats = self.scraper.get_player_advanced_stats(year)
#         self.save_to_csv(player_advanced_stats, f"player_advanced_stats_{year}.csv")

#         player_shooting_stats = self.scraper.get_player_shooting_stats(year)
#         self.save_to_csv(player_shooting_stats, f"player_shooting_stats_{year}.csv")

#         team_ratings = self.scraper.get_team_ratings(year)
#         self.save_to_csv(team_ratings, f"team_ratings_{year}.csv")

#         all_games = self.scraper.get_all_games_current_season(f"{year}-01-01", f"{year}-12-31")
#         self.save_to_csv(all_games, f"all_games_{year}.csv")

#         team_standings = self.scraper.get_team_standings(year)
#         self.save_to_csv(team_standings, f"team_standings_{year}.csv")

#         player_points_total = self.scraper.get_player_points_total(year)
#         self.save_to_csv(player_points_total, f"player_points_total_{year}.csv")
    
#     def scrape_all_years(self, start_year=2022):
#         current_year = datetime.now().year
#         for year in range(start_year, current_year + 1):
#             self.scrape_all_years(year)

# if __name__ == "__main__":
#     local_scraper = LocalScraper()
#     local_scraper.scrape_all_years()

#     print("Data has been saved to the data folder.")
...
class TestScraper:
    ...
    def test_scrape(self):
        ...
        # print("Testing injury report...")
        # injuries = self.scraper.get_injury_report()
        # print(injuries.head())

        # print("Testing player per game stats...")
        # player_per_game_stats = self.scraper.get_player_per_game_stats(year)
        # print(player_per_game_stats.head())

        # print("Testing player per 36 stats...")
        # player_per_36_stats = self.scraper.get_player_36_stats(year)
        # print(player_per_36_stats.head())

        # print("Testing player per 100 stats...")
        # player_per_100_stats = self.scraper.get_player_100_stats(year)
        # print(player_per_100_stats.head())

        # print("Testing player play by play stats...")
        # player_play_by_play_stats = self.scraper.get_player_play_by_play_stats(year)
        # print(player_play_by_play_stats.head())

        # print("Testing player advanced stats...")
        # player_advanced_stats = self.scraper.get_player_advanced_stats(year)
        # print(player_advanced_stats.head())

        # print("Testing player shooting stats...")
        # player_shooting_stats = self.scraper.get_player_shooting_stats(year)
        # print(player_shooting_stats.head())

        # print("Testing team ratings...")
        # team_ratings = self.scraper.get_team_ratings(year)
        # print(team_ratings.head())

if __name__ == "__main__":
    test_scraper = TestScraper()
    test_scraper.test_scrape()